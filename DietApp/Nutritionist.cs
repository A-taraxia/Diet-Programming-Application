    ...
    public class Nutritionist : Users
    {
        ...
        public void createNewplan(DietRequirements plan, BMI bmi, string boolean_special_needs, string boolean_exclude)
        {
            ...
            else
            {
                ...
                DatabaseManager.updateData("UPDATE special_needs SET cancer = '" + string_list[0] + "', diabetes = '" + string_list[1] + "', hyperthyroidism = '" + string_list[2] + "', hypothyroidism = '" + string_list[3] + "', high_blood_cholesterol = '" + string_list[4] + "', high_blood_sugar = '" + string_list[5] + "', pregnacy = '" + string_list[6] + "', breast_feeding = '" + string_list[7] + "', cardiovascular_diseases = '" + string_list[8] + "', osteoporosis = '" + string_list[9] + "' WHERE patient_id = '" + plan.patient_id + "'");
        ...
        public void createWeeklydiet(DietRequirements program, BMI bmi, Patient patient)
        {
            ...
            for (int i = 0; i <= 6; i++)
            {
                ...
                // data of daily foods
                List<List<string>> daily_main_dishes = DatabaseManager.returnData("select kcal,fats,protein,carbs,includes from food where foodname='" + breakfast_name + "' or foodname='" + lunch_name + "' or foodname='" + dinner_name + "'");
                List<string> daily_snack = DatabaseManager.returnData("select kcal,fats,protein,carbs,includes from food where foodname='" + snack_name + "'")[0];
